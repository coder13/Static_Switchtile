...
document.addEventListener("touchsmove", function(e){e.preventDefault();}, false);
...
var browser = util.getBrowser(); // only want to call this once
...
function pretty(time) {
    ...
    var secs = util.trim((time - 60000*mins)/1000, 3);
    if (mins == 0) {
        return secs;
    } else {
...
function displayTimes(loadedPage, time) {
    ...
    if (!times[n])
        times[n] = [];
    for (let i = 1; i < times[n].length; i++) {
    ...
    for (let i = 0; i < avgLengths.length; i++) {
        ...
        if (times[n].length >= len) {
            ...
            if (loadedPage) {
                ...
                for (let j=0; j <= times[n].length-len; j++) {
